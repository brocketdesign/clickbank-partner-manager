...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  try {
    const doc = await db.collection("partners").findOne({ _id: new ObjectId(id) });
...
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  if ("partner_name" in body) update.partner_name = (body.partner_name as string)?.trim();
  if ("aff_id" in body) update.aff_id = (body.aff_id as string)?.trim();
